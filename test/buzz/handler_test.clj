...
(defn- captured-out
  ...
  print is captured too. `with-out-str` only binds it on this one."
  [f]
...
;; The failure no part of the mechanism can see: a slot reads mutable state
;; without going through a source, so the connection holds nothing for it and
;; nothing ever marks it. The check finds it from the outside.
(defonce ^:private untracked (atom 0))

(defui unwatched []
  [:p (server @untracked)])

(deftest the-check-finds-a-slot-that-reads-outside-a-source
  (reset! untracked 0)
  (with-connection {:mounts [{:el "app" :ui #'unwatched}]}
    (fn [{:keys [sock rdr]}]
      (is (= ["mount" "unwatched" "app" [0]] (next-event rdr)))

      (testing "without the check, a write reaches nobody"
        (swap! untracked inc)
        (is (silent? sock rdr 300)))

      (testing "with the check, the connection is patched and the miss is named"
        (let [out (captured-out
                   (fn []
                     (handler/check-topics! true 100)
                     (Thread/sleep 400)
                     (handler/check-topics! false)))]
          (is (str/includes? out "missed update"))
          (is (str/includes? out "no source it reads said so"))))

      (testing "and the browser has the value it was missing"
        (is (= ["patch" "unwatched" [1]] (next-event rdr))))

      (testing "the check stops when it is turned off"
        (swap! untracked inc)
        (is (silent? sock rdr 300))))))

(deftest the-check-is-quiet-when-every-read-goes-through-a-source
  (reset! ledger {"alice" ["water the plants"]})
  (with-connection {:mounts [{:el "app" :ui #'observed-notes}]}
    (fn [{:keys [rdr]}]
      (next-event rdr)
      (let [out (captured-out
                 (fn []
                   ...
                   (Thread/sleep 400)
                   (handler/check-topics! false)))]
        (is (not (str/includes? out "missed update")) out)))))
